...
import { AceitoCard, EnviadoCard, RecusadoCard, RespondidoCard } from "./PropostaCard";
    ...
    <div className="flex h-full space-x-4">
      ...
      <div className="flex-none w-80 bg-blue-100 p-4 rounded-md shadow-md flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          {enviados.length === 0 && <p className="text-gray-500">Nenhuma proposta enviada.</p>}
          {enviados.map((p, index) => (
            <EnviadoCard proj={p} key={index} />
          ))}
        </div>
      </div>
      ...
      <div className="flex-none w-80 bg-green-100 p-4 rounded-md shadow-md flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          {respondidos.length === 0 && <p className="text-gray-500">Nenhuma proposta respondida.</p>}
          {respondidos.map((p, index) => (
            <RespondidoCard proj={p} key={index} />
          ))}
        </div>
      </div>
      ...
      <div className="flex-none w-80 bg-yellow-100 p-4 rounded-md shadow-md flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          {aceitos.length === 0 && <p className="text-gray-500">Nenhuma proposta aceita.</p>}
          {aceitos.map((p, index) => (
            <AceitoCard proj={p} key={index} />
          ))}
        </div>
      </div>
      ...
      <div className="flex-none w-80 bg-red-100 p-4 rounded-md shadow-md flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          {recusados.length === 0 && <p className="text-gray-500">Nenhuma proposta recusada.</p>}
          {recusados.map((p, index) => (
            <RecusadoCard proj={p} key={index} />
          ))}
        </div>
      </div>
    </div>
